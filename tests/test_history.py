...
async def create_admin_and_get_token(client: AsyncClient, db_session: AsyncSession, email: str, password: str) -> str:
...
async def test_history_endpoints_unauthenticated(client: AsyncClient) -> None:
    """Verify that history endpoints return 401 Unauthorized when no credentials are provided."""
    # List history
...
@pytest.mark.anyio
async def test_conversion_records_user_id_if_authenticated(client: AsyncClient, db_session: AsyncSession) -> None:
    """Verify that performing a conversion registers the user's ID if they are authenticated."""
    token = await create_user_and_get_token(client, "user_conv@example.com", "password123")
    headers = {"Authorization": f"Bearer {token}"}

    # Perform a conversion
    response = await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=100", headers=headers)
...
@pytest.mark.anyio
async def test_history_filtering_sorting_and_pagination(client: AsyncClient, db_session: AsyncSession) -> None:
    """Verify listing history supports pagination, filters, and custom sorting."""
    # 1. Setup users
    user_a_token = await create_user_and_get_token(client, "usera@example.com", "password123")
    user_b_token = await create_user_and_get_token(client, "userb@example.com", "password123")
    ...
    conv_a1 = await client.get("/api/v1/currencies/convert?from=USD&to=EUR&amount=100", headers=headers_a)
    conv_a2 = await client.get("/api/v1/currencies/convert?from=USD&to=GBP&amount=200", headers=headers_a)
    conv_a3 = await client.get("/api/v1/currencies/convert?from=USD&to=EUR&amount=50", headers=headers_a)
    ...
    conv_b1 = await client.get("/api/v1/currencies/convert?from=USD&to=EUR&amount=500", headers=headers_b)
    ...
    assert all(item["user_id"] == 1 for item in data["items"])  # Assuming User A id is 1
    ...
    resp = await client.get("/api/v1/history?sort_by=amount&sort_order=asc", headers=headers_a)
    assert resp.status_code == 200
    ...
    # Sort by amount desc:
    resp = await client.get("/api/v1/history?sort_by=amount&sort_order=desc", headers=headers_a)
    assert resp.status_code == 200
    items = resp.json()["items"]
    ...
    resp = await client.get(f"/api/v1/history?start_date={future_date}", headers=headers_a)
    assert resp.status_code == 200
    assert resp.json()["total"] == 0

    # Start date in the past should yield all
    past_date = (conversion_time - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%SZ")
    resp = await client.get(f"/api/v1/history?start_date={past_date}", headers=headers_a)
    assert resp.status_code == 200
    assert resp.json()["total"] == 3


@pytest.mark.anyio
async def test_admin_global_history_access(client: AsyncClient, db_session: AsyncSession) -> None:
    """Verify that admins can view global history and filter by user_id."""
    user_token = await create_user_and_get_token(client, "user_hist@example.com", "password123")
    admin_token = await create_admin_and_get_token(client, db_session, "admin_hist@example.com", "admin123")

    headers_user = {"Authorization": f"Bearer {user_token}"}
    headers_admin = {"Authorization": f"Bearer {admin_token}"}

    # Perform conversions
    await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=10", headers=headers_user)
    await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=20", headers=headers_admin)
    ...
    resp = await client.get(f"/api/v1/history?user_id={user_obj.id}", headers=headers_admin)
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_get_single_history_record_authorization(client: AsyncClient, db_session: AsyncSession) -> None:
    """Verify single record retrieval logic and route-level authorization."""
    user_a_token = await create_user_and_get_token(client, "usera_auth@example.com", "password123")
    user_b_token = await create_user_and_get_token(client, "userb_auth@example.com", "password123")
    admin_token = await create_admin_and_get_token(client, db_session, "admin_auth@example.com", "admin123")
    ...
    conv_resp = await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=150", headers=headers_a)
...
@pytest.mark.anyio
async def test_delete_history_record_authorization(client: AsyncClient, db_session: AsyncSession) -> None:
    """Verify single record deletion logic and route-level authorization."""
    user_a_token = await create_user_and_get_token(client, "usera_del@example.com", "password123")
    user_b_token = await create_user_and_get_token(client, "userb_del@example.com", "password123")
    admin_token = await create_admin_and_get_token(client, db_session, "admin_del@example.com", "admin123")
    ...
    conv1 = await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=10", headers=headers_a)
    conv2 = await client.get("/api/v1/currencies/convert?from=USD&to=USD&amount=20", headers=headers_a)
    id1 = conv1.json()["id"]
    id2 = conv2.json()["id"]
