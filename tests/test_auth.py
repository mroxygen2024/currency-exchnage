...
async def test_login_invalid_credentials(client: AsyncClient) -> None:
    ...
    # Non-existent user
    login_payload_missing = {
        "username": "missing@example.com",
        "password": "password123",
    }
    login_resp_missing = await client.post(
        "/api/v1/auth/login", data=login_payload_missing
    )
...
async def test_token_refresh_flow(client: AsyncClient) -> None:
    ...
    failed_refresh_resp = await client.post(
        "/api/v1/auth/refresh", json={"refresh_token": first_refresh_token}
    )
...
async def test_logout_revocation(client: AsyncClient) -> None:
    """Verify that logging out revokes the refresh token and prevents
    future refreshes.
    """
